<?php namespace Application\Block\Features;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btFeatures';
    protected $btExportTables = ['btFeatures', 'btFeaturesEntries'];
    ...
    public function addEdit() {
        ...
        // Link Address (link) - Fields that don't exist in database, but are required in repeatable entry (link)
        ...
        $al->register('css', 'features/form', 'blocks/features/css_files/form.css', [], false);
        $this->requireAsset('css', 'features/form');
    ...
    public function save($args) {
        ...
        $db->delete('btFeaturesEntries', ['bID' => $this->bID]);
        ...
        if (isset($args['entry']) AND is_array($args['entry']) AND count($args['entry'])) {
            ...
            foreach ($args['entry'] as $entry) {
                ...
                $db->insert('btFeaturesEntries', $data);
    ...
    public function composer() {
        ...
        $al = AssetList::getInstance();
        $al->register('javascript', 'features/auto-js', 'blocks/features/auto.js', [], false);
        $this->requireAsset('javascript', 'features/auto-js');
    ...
    private function getEntries($outputMethod = 'view') {
        ...
        $sql = '
            ...
            WHERE
                btFeaturesEntries.bID = :bID
            ORDER BY
                btFeaturesEntries.position ASC
        ';
        $parameters = [];
        $parameters['bID'] = $this->bID;
     ...
     private function getEntryColumnNames() {
        ...
        $columns = $db->getSchemaManager()->listTableColumns('btFeaturesEntries');
