<?php namespace Application\Block\ServiceHighlight;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btServiceHighlight';
    protected $btExportTables = ['btServiceHighlight', 'btServiceHighlightEntries'];
    ...
    public function addEdit() {
        ...
        $entryColumnNames[] = 'hover_image_override_dimensions';
        $entryColumnNames[] = 'hover_image_custom_width';
        $entryColumnNames[] = 'hover_image_custom_height';
        $entryColumnNames[] = 'hover_image_custom_crop';
        $entryColumnNames[] = 'hover_image_override_fullscreen_dimensions';
        $entryColumnNames[] = 'hover_image_custom_fullscreen_width';
        $entryColumnNames[] = 'hover_image_custom_fullscreen_height';
        $entryColumnNames[] = 'hover_image_custom_fullscreen_crop';

        $this->set('entryColumnNames', $entryColumnNames);

        // Load form.css
        $al = AssetList::getInstance();
        $al->register('css', 'service-highlight/form', 'blocks/service_highlight/css_files/form.css', [], false);
        $this->requireAsset('css', 'service-highlight/form');
    ...
    public function save($args) {
        ...
        if (isset($args['settings']) and $args['settings']['hover_image_custom_crop']==='1' and (empty($args['settings']['hover_image_custom_width']) or empty($args['settings']['hover_image_custom_height']))) {
            $args['settings']['hover_image_custom_crop'] = 0; // Crop should be disabled if width or height is missing
        }
        if (isset($args['settings']) and $args['settings']['hover_image_custom_fullscreen_crop']==='1' and (empty($args['settings']['hover_image_custom_fullscreen_width']) or empty($args['settings']['hover_image_custom_fullscreen_height']))) {
            $args['settings']['hover_image_custom_fullscreen_crop'] = 0; // Crop should be disabled if width or height is missing
        }
        ...
        $db->delete('btServiceHighlightEntries', ['bID' => $this->bID]);
        ...
        if (isset($args['entry']) AND is_array($args['entry']) AND count($args['entry'])) {
            ...
            foreach ($args['entry'] as $entry) {
                ...
                $data['hover_image_data'] = json_encode([
                    ...
                    'override_dimensions'            => !empty($entry['hover_image_override_dimensions']) ? intval($entry['hover_image_override_dimensions']) : 0,
                    'custom_width'                   => intval($entry['hover_image_custom_width']),
                    'custom_height'                  => intval($entry['hover_image_custom_height']),
                    'custom_crop'                    => ($entry['hover_image_custom_crop']==='1' and (!(bool)$entry['hover_image_custom_width'] or !(bool)$entry['hover_image_custom_height'])) ? false : intval($entry['hover_image_custom_crop']), // do not crop without width and height filled
                    'override_fullscreen_dimensions' => !empty($entry['hover_image_override_fullscreen_dimensions']) ? intval($entry['hover_image_override_fullscreen_dimensions']) : 0,
                    'custom_fullscreen_width'        => intval($entry['hover_image_custom_fullscreen_width']),
                    'custom_fullscreen_height'       => intval($entry['hover_image_custom_fullscreen_height']),
                    'custom_fullscreen_crop'         => ($entry['hover_image_custom_fullscreen_crop']==='1' and (!(bool)$entry['hover_image_custom_fullscreen_width'] or !(bool)$entry['hover_image_custom_fullscreen_height'])) ? false : intval($entry['hover_image_custom_fullscreen_crop']), // do not crop without width and height filled
                ]);

                $db->insert('btServiceHighlightEntries', $data);
    ...
    public function duplicate($newBlockID) {
        ...
        $sql = '
            SELECT
                btServiceHighlightEntries.*
            FROM
                btServiceHighlightEntries
            WHERE
                btServiceHighlightEntries.bID = :bID
        ';
        $parameters = [];
    ...
    public function composer() {
        ...
        $al = AssetList::getInstance();
        $al->register('javascript', 'service-highlight/auto-js', 'blocks/service_highlight/auto.js', [], false);
        $this->requireAsset('javascript', 'service-highlight/auto-js');
    ...
    private function getEntries($outputMethod = 'view') {
        ...
        $sql = '
            ...
            WHERE
                btServiceHighlightEntries.bID = :bID
            ORDER BY
                btServiceHighlightEntries.position ASC
        ';
        $parameters = [];
        $parameters['bID'] = $this->bID;
        ...
        foreach ($entries as $entry) {
            ...
            $entry['hover_image_override_dimensions']            = $hover_imageArray['override_dimensions'] ?? '';
            $entry['hover_image_custom_width']                   = $hover_imageArray['custom_width'] ?? '';
            $entry['hover_image_custom_height']                  = $hover_imageArray['custom_height'] ?? '';
            $entry['hover_image_custom_crop']                    = $hover_imageArray['custom_crop'] ?? '';
            $entry['hover_image_override_fullscreen_dimensions'] = $hover_imageArray['override_fullscreen_dimensions'] ?? '';
            $entry['hover_image_custom_fullscreen_width']        = $hover_imageArray['custom_fullscreen_width'] ?? '';
            $entry['hover_image_custom_fullscreen_height']       = $hover_imageArray['custom_fullscreen_height'] ?? '';
            $entry['hover_image_custom_fullscreen_crop']         = $hover_imageArray['custom_fullscreen_crop'] ?? '';
            // Link (link) - Link
     ...
     private function getEntryColumnNames() {
        ...
        $columns = $db->getSchemaManager()->listTableColumns('btServiceHighlightEntries');
    ...
    private function prepareEntriesForView($entries) {
        ...
        if (is_array($entries) AND count($entries)) {
            ...
            foreach ($entries as $key => $entry) {
                ...
                // Hover Image (hover_image) - Image
                $thumbnailWidth = 480;
                if (!empty($this->settings['hover_image_override_dimensions'])) {
                    $thumbnailWidth = !empty($this->settings['hover_image_custom_width']) ? $this->settings['hover_image_custom_width'] : false;
                }
                if (!empty($entry['hover_image_override_dimensions'])) {
                    $thumbnailWidth = !empty($entry['hover_image_custom_width']) ? $entry['hover_image_custom_width'] : false;
                }
                $thumbnailHeight = 270;
                if (!empty($this->settings['hover_image_override_dimensions'])) {
                    $thumbnailHeight = !empty($this->settings['hover_image_custom_height']) ? $this->settings['hover_image_custom_height'] : false;
                }
                if (!empty($entry['hover_image_override_dimensions'])) {
                    $thumbnailHeight = !empty($entry['hover_image_custom_height']) ? $entry['hover_image_custom_height'] : false;
                }
                $thumbnailCrop = true;
                if (!empty($this->settings['hover_image_override_dimensions'])) {
                    $thumbnailCrop = !empty($this->settings['hover_image_custom_crop']) ? $this->settings['hover_image_custom_crop'] : false;
                }
                if (!empty($entry['hover_image_override_dimensions'])) {
                    $thumbnailCrop = !empty($entry['hover_image_custom_crop']) ? $entry['hover_image_custom_crop'] : false;
                }
                $fullscreenWidth = 1920;
                if (!empty($this->settings['hover_image_override_fullscreen_dimensions'])) {
                    $fullscreenWidth = !empty($this->settings['hover_image_custom_fullscreen_width']) ? $this->settings['hover_image_custom_fullscreen_width'] : false;
                }
                if (!empty($entry['hover_image_override_fullscreen_dimensions'])) {
                    $fullscreenWidth = !empty($entry['hover_image_custom_fullscreen_width']) ? $entry['hover_image_custom_fullscreen_width'] : false;
                }
                $fullscreenHeight = 1080;
                if (!empty($this->settings['hover_image_override_fullscreen_dimensions'])) {
                    $fullscreenHeight = !empty($this->settings['hover_image_custom_fullscreen_height']) ? $this->settings['hover_image_custom_fullscreen_height'] : false;
                }
                if (!empty($entry['hover_image_override_fullscreen_dimensions'])) {
                    $fullscreenHeight = !empty($entry['hover_image_custom_fullscreen_height']) ? $entry['hover_image_custom_fullscreen_height'] : false;
                }
                $fullscreenCrop = false;
                if (!empty($this->settings['hover_image_override_fullscreen_dimensions'])) {
                    $fullscreenCrop = !empty($this->settings['hover_image_custom_fullscreen_crop']) ? $this->settings['hover_image_custom_fullscreen_crop'] : false;
                }
                if (!empty($entry['hover_image_override_fullscreen_dimensions'])) {
                    $fullscreenCrop = !empty($entry['hover_image_custom_fullscreen_crop']) ? $entry['hover_image_custom_fullscreen_crop'] : false;
                }
                $modifiedEntry = $this->prepareForViewImage('entry', [
