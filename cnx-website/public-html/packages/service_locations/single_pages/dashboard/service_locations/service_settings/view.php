...
$userInterface = $app->make(\Concrete\Core\Application\Service\UserInterface::class);

echo $userInterface->tabs([
    ['csv-variables', t('CSV Variables'), true],
    ['import', t('Import')],
    ['content-settings', t('Content Settings')],
]);
?>
...
<div class="tab-pane active"  id="csv-variables">
...
<div class="row">
    ...
    <div class="res-message variable-error">
			...
			<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close" onclick="$('.alert').alert('close')" >&times;</a>
		</div>
    </div>
...
<div class="tab-pane" role="tabpanel" id="import">
  ...
  <div class="clearfi clear-bottomx">
    ...
    <div class="input"> <?php echo $al->file('ccm-b-file', 'fID', t('Choose File'), isset($bf) ? $bf : '') ?> </div>
  </div>
...
<div class="tab-pane" role="tabpanel" id="content-settings">
  <div class="clearfix clear-bottom">
  ...
  <div class="clearfix clear-bottom">
	...
	echo $editor->outputEditorWithOptions('defaultContent',array('height'=>'500'));
	?>
    </div>
  </div>
  ...
  <div class="ccm-pane-footer">
    ...
    <?php print $ih->submit('Save Settings ', 'settings-form', 'right', 'btn-primary');?> <?php print $ih->button(t('Cancel'), $this->url('/dashboard/pixel_module_management/list'), 'left');?>
  </div>
...
if($.trim(obj.service_content)!=''){
	CKEDITOR.instances[$('#content-settings textarea').attr('id')].setData(obj.service_content);
}else{
//	$('.default-content .cke_wysiwyg_div').html("");
}
