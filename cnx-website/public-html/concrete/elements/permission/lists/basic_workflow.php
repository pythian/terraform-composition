...
<?php if ($enablePermissions) {
	...
	ccm_permissionLaunchDialog = function(link) {
		...
		jQuery.fn.dialog.open({
			...
			href: CCM_DISPATCHER_FILENAME + '/ccm/system/dialogs/permissions/basic_workflow?duplicate=' + dupe + '&wfID=<?=$workflow->getWorkflowID()?>&pkID=' + $(link).attr('data-pkID') + '&paID=' + $(link).attr('data-paID'),
			modal: true,
			width: 500,
			height: 380
		});
