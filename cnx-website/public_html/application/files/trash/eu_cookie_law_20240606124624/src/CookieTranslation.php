...
class CookieTranslation
{
    ...
    {
        return array("ar_SA", "en_US", "de_DE", "bg_BG", "hr_HR", "cs_CZ", "da_DK", "nl_NL", "et_EE", "fi_FL", "fr_FR", "el_GR", "hu_HU", "en_IE", "it_IT", "lv_LV", "lt_LT", "mt_MT", "pl_PL", "pt_PT", "ro_RO", "sk_SK", "sl_SI", "es_ES", "sv_FI");
    }
    ...
    public function getLocale()
    {
                ...
                foreach ($this->getUsersPreferedLocale() as $locale => $priority) {
                    ...
                    if ($this->isValidLocale($locale)) {
                        ...
                        foreach ($this->getSupportedLocales() as $supportedLocale) {
                            $supportedLocaleShort = substr($supportedLocale, 0, 2);

                            if (strtoupper($supportedLocaleShort) === strtoupper($shortLocale)) {
        ...
        } elseif ($settings->getLanguage() == Languages::GEO) {
            $countryCode = Helpers::getCountryCodeFromIpAddress();

            foreach ($this->getSupportedLocales() as $supportedLocale) {
                $supportedLocaleShort = substr($supportedLocale, 0, 2);

                if (strtoupper($supportedLocaleShort) === strtoupper($countryCode)) {
                    return $supportedLocale;
                }
            }

            return $this->getDefaultLocale();
        } elseif ($settings->getLanguage() == Languages::LANGUAGE_SETTING) {
            $countryCode = Localization::activeLanguage();
