...
$sub_content='';
                                if ($children && !$ni->cObj->getAttribute('exclude_subpages_from_nav')) {
                                    ...
                                    foreach ($children as $k => $childID) {
                                        ...
                                        $target = '_self';
                if ($child->getCollectionPointerExternalLink() != '') {
                    $url = $child->getCollectionPointerExternalLink();
                    if ($child->openCollectionPointerExternalLinkInNewWindow()) {
                        $target = '_blank';
                    }
                } else {
                    $url = $child->getCollectionLink();
                    $target = $child->getAttribute('nav_target');
                }
                                        ?>

                                        <li class="mega_item <?= currentPage($childID,$pageID)?>" data-id="<?=$childID?>-<?= $k+1; ?>"><a href="<?= $url; ?>" target="<?= $target; ?>" title="<?= $child->getCollectionName(); ?>"><?= $child->getCollectionName(); ?></a></li>
...
if($ckey==0) { $sub_content.='<div class="col-12 col-md-4"><ul class="nav">'; }elseif($ckey%$totalNav==0){ $sub_content.=' </ul></div><div class="col-12 col-md-4"><ul class="nav">';  }
                                                ...
                                                if($childObj->getAttribute('exclude_nav'))continue;
                                                $target = '_self';
                                                if ($childObj->getCollectionPointerExternalLink() != '') {
                                                    $url = $childObj->getCollectionPointerExternalLink();
                                                    if ($childObj->openCollectionPointerExternalLinkInNewWindow()) {
                                                        $target = '_blank';
                                                    }
                                                } else {
                                                    $url = $childObj->getCollectionLink();
                                                    $target = $childObj->getAttribute('nav_target');
                                                }
            ...
            $sub_content.='<li class="'.currentPage($childID,$pageID).'" ><a href="'.$url.'" target="'.$target.'" title="'.$childObj->getCollectionName().'">'.$childObj->getCollectionName().'</a>';
...
$sub_content.='<li class="'.currentPage($cchildID,$pageID).'" ><a href="'.$url.'" target="'.$target.'" title="'.$child->getCollectionName().'">'.$child->getCollectionName().'</a></li>';
...
<?php } else {
   ...
   if ($children) {
    ...
    foreach ($children as $childID) {
        ...
        <li class="<?= currentPage($childID,$pageID)?>"><a href="<?= $url; ?>" target="<?= $target; ?>" title="<?= $child->getCollectionName(); ?>"><?= $child->getCollectionName(); ?></a></li>
