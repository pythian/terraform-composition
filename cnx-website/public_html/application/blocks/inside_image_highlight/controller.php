<?php namespace Application\Block\InsideImageHighlight;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btInsideImageHighlight';
    protected $btExportTables = ['btInsideImageHighlight', 'btInsideImageHighlightEntries'];
    ...
    public function getBlockTypeName() {
        return t('Inside Image Highlight');
    }
    ...
    public function on_start() {
        ...
        // Entry / Image Position (image_position) options
        $entry_image_position_options      = [];
        $entry_image_position_options[]    = '----';
        $entry_image_position_options['1'] = t('Left');
        $entry_image_position_options['2'] = t('Right');

        $this->set('entry_image_position_options', $entry_image_position_options);
    ...
    public function addEdit() {
        ...
        $al->register('css', 'inside-image-highlight/form', 'blocks/inside_image_highlight/css_files/form.css', [], false);
        $this->requireAsset('css', 'inside-image-highlight/form');
    ...
    public function save($args) {
        ...
        $db->delete('btInsideImageHighlightEntries', ['bID' => $this->bID]);
        ...
        if (isset($args['entry']) AND is_array($args['entry']) AND count($args['entry'])) {
            ...
            foreach ($args['entry'] as $entry) {
                ...
                $data['image_position'] = !empty($entry['image_position']) ? trim($entry['image_position']) : '';
                ...
                $db->insert('btInsideImageHighlightEntries', $data);
    ...
    public function duplicate($newBlockID) {
        ...
        $sql = '
            SELECT
                btInsideImageHighlightEntries.*
            FROM
                btInsideImageHighlightEntries
            WHERE
                btInsideImageHighlightEntries.bID = :bID
        ';
        $parameters = [];
    ...
    public function composer() {
        ...
        $al = AssetList::getInstance();
        $al->register('javascript', 'inside-image-highlight/auto-js', 'blocks/inside_image_highlight/auto.js', [], false);
        $this->requireAsset('javascript', 'inside-image-highlight/auto-js');
    ...
    private function getEntries($outputMethod = 'view') {
        ...
        $sql = '
            ...
            WHERE
                btInsideImageHighlightEntries.bID = :bID
            ORDER BY
                btInsideImageHighlightEntries.position ASC
        ';
        $parameters = [];
        $parameters['bID'] = $this->bID;
     ...
     private function getEntryColumnNames() {
        ...
        $columns = $db->getSchemaManager()->listTableColumns('btInsideImageHighlightEntries');
